...
function buildEmail(type, ctx, cfg) {
  const sup = ctx.sup || {};
  const to = sup.email || "", greeting = sup.contactName || sup.name || "Partner";
  if (type === "licence") {
    const days = ctx.daysLeft, expired = days < 0;
    return {
      to,
      toName: greeting,
      greeting,
...
// _src/cron-licence.mjs
var config = { schedule: "0 6 * * *" };
var cron_licence_default = async () => {
  try {
    const res = await runLicenceJob(store());
...
export {
  config,
  cron_licence_default as default
};
