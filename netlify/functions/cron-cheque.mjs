...
async function runChequeReminderJob(s) {
  const cfg = await getEmailCfg(s);
  const register = await s.get("register", { type: "json" }) || [];
  const settings = await s.get("settings", { type: "json" }) || {};
  const lead = num(settings.chequeReminderLeadDays) || 2;
  const log = settings.chequeReminders || {};
  const due = [];
  for (const r of register) {
    if (!r || r.mode !== "Cheque") continue;
    const d = daysUntil(r.date);
    if (d === null || d < 0 || d > lead) continue;
    const key = `${r.no || ""}|${String(r.ref || "").trim()}|${r.date}`;
    if (log[key]) continue;
    due.push({ r, d, key });
  }
  let pruned = 0;
  ...
  if (!due.length) {
    if (pruned) {
      settings.chequeReminders = log;
      try {
        await s.setJSON("settings", settings);
      } catch {
      }
    }
    return { job: "cheque", lead, due: 0, pruned };
  }
  due.sort((a, b) => a.r.date < b.r.date ? -1 : a.r.date > b.r.date ? 1 : 0);
  const to = settings.chequeReminderTo || cfg.adminEmail;
  const total = due.reduce((t, x) => t + num(x.r.amount), 0);
  const label = (d) => d === 0 ? "TODAY" : d === 1 ? "tomorrow" : `in ${d} days`;
  const table = due.map((x) => [
    `${emDate(x.r.date)} (${label(x.d)})`,
    `Chq ${String(x.r.ref || "\u2014")} \xB7 ${x.r.payee || x.r.supplier || "\u2014"}${x.r.project ? " \xB7 " + x.r.project : ""} \xB7 ${emMoney(x.r.amount)}`
  ]);
  table.push(["Total falling due", emMoney(total)]);
  const bankSet = {};
  for (const x of due) {
    const b = x.r.bank || "\u2014";
    bankSet[b] = (bankSet[b] || 0) + num(x.r.amount);
  }
  const byBank = Object.entries(bankSet).map(([b, v]) => `${emEsc(b)}: ${emMoney(v)}`).join(" &nbsp;\u2022&nbsp; ");
  const html = emailShell(cfg, {
    title: `Cheque Reminder \u2014 ${due.length} cheque${due.length === 1 ? "" : "s"} due within ${lead} day${lead === 1 ? "" : "s"}`,
    band: "#c0392b",
    greeting: "Mohammed",
    preheader: `${due.length} cheque(s) totalling ${emMoney(total)} due within ${lead} days.`,
    lead: [
      `This is an automated reminder that the following issued cheque${due.length === 1 ? " is" : "s are"} due to be presented within the next <strong>${lead} day${lead === 1 ? "" : "s"}</strong>. Please ensure cleared funds are available in the relevant account${Object.keys(bankSet).length === 1 ? "" : "s"} before the cheque date.`
    ],
    table,
    note: `<strong>Funds required by account:</strong> ${byBank}. Kindly arrange the balance so no cheque is returned.`,
    noteColor: "#fdecea",
    noteBar: "#c0392b",
    closing: `You are receiving this because a cheque reminder ${lead} day(s) in advance is enabled.`
  });
  const rec = await sendMail(s, cfg, { type: "chequereminder", to, toName: "MA Group", subject: `Cheque Reminder \u2014 ${due.length} due within ${lead} day${lead === 1 ? "" : "s"} (${emMoney(total)})`, html });
  const today = now().slice(0, 10);
  for (const x of due) log[x.key] = today;
  settings.chequeReminders = log;
  try {
    await s.setJSON("settings", settings);
  } catch {
  }
  return { job: "cheque", lead, due: due.length, total: r2(total), to, status: rec?.status, pruned };
}

// _src/cron-cheque.mjs
var config = { schedule: "0 4 * * *" };
var cron_cheque_default = async () => {
  try {
    const res = await runChequeReminderJob(store());
...
export {
  config,
  cron_cheque_default as default
};
