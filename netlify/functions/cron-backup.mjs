...
async function runBackupJob(s) {
  const cfg = await getEmailCfg(s);
  const allJSON = async (p) => {
  ...
  const data = {};
  for (const p of prefixes) data[p.replace(/\//g, "")] = await allJSON(p);
  ...
  data.users = us.map((u) => ({ id: u.id, name: u.name, role: u.role }));
  const counts = {};
  for (const k in data) counts[k] = Array.isArray(data[k]) ? data[k].length : 1;
  const payload = JSON.stringify({ generatedAt: now(), version: 1, counts, data });
  const to = getEnv("BACKUP_TO") || getEnv("PNL_TO") || "ceo@maagroup.ae";
  const fname = "MA_Group_Finance_backup_" + now().slice(0, 10) + ".json";
  const summ = Object.entries(counts).map(([k, v]) => [k, String(v)]);
  const kb = (payload.length / 1024).toFixed(0);
  const html = emailShell(cfg, {
    title: "Weekly Data Backup \u2014 MA Group Finance",
    band: "#375623",
    greeting: "Mohammed",
    preheader: `Automated database backup attached (${kb} KB).`,
    lead: [`Attached is the automated weekly backup of the MA Group Finance database as of <strong>${emDate(now())}</strong>.`, `Please keep a copy off-system (e.g. Google Drive or an external disk). It contains every record; PINs are never included.`],
    table: summ,
    note: `To restore from this file, contact your system administrator. Backups are also retained in Netlify's replicated cloud storage.`,
    noteColor: "#eef7ee",
    noteBar: "#375623",
    closing: `This backup runs automatically every Monday. You can also download one any time from Settings \u2192 Backup.`
  });
  const rec = await sendMail(s, cfg, { type: "backup_weekly", to, toName: "Mohammed Abuassba", subject: `Weekly Backup \u2014 MA Group Finance (${now().slice(0, 10)})`, html, attachments: [{ filename: fname, content: payload, contentType: "application/json" }] });
  return { job: "backup", counts, bytes: payload.length, status: rec?.status };
}

// _src/cron-backup.mjs
var config = { schedule: "0 3 * * 1" };
var cron_backup_default = async () => {
  try {
    const res = await runBackupJob(store());
...
export {
  config,
  cron_backup_default as default
};
